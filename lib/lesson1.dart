...
int seconds(int hours, int minutes, int seconds) {
  inputNullValueChecker([hours, minutes, seconds]);
  ...
  inputOutOfRangeChecker(hoursOutOfRange || minutesOutOfRange || secondsOutOfRange);
...
double lengthInMeters(int sagenes, int arshins, int vershoks) {
  inputIsNaturalChecker([sagenes, arshins, vershoks]);
...
int travelMinutes(
  ...
  inputNullValueChecker([hoursDepart, hoursArrive, minutesDepart, minutesArrive]);
  ...
  inputOutOfRangeChecker(hoursDepartOutOfRange || hoursArriveOutOfRange || minutesDepartOutOfRange || minutesArriveOutOfRange);
  inputWrongStateChecker(hoursDepart > hoursArrive || hoursArrive == hoursDepart && minutesDepart > minutesArrive);
...
double accountInThreeYears(int initial, int percent) {
  inputIsNaturalChecker([initial, percent]);
...
int numberRevert(int number) {
  ...
  inputOutOfRangeChecker(number < 100 || number > 999);
...
bool isNumberHappy(int number) {
  inputNullValueChecker([number]);
  inputOutOfRangeChecker(number < 1000 || number > 9999);
...
bool queenThreatens(int x1, int y1, int x2, int y2) {
  inputNullValueChecker([x1, x2, y1, y2]);
...
int daysInMonth(int month, int year) {
  inputNullValueChecker([month, year]);
  inputOutOfRangeChecker(year < 0 || month < 0 || month > 12);
